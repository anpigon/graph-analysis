...
const CoCitations: React.FC<CoCitationsProps> = ({
  ...
  const loadMoreData = () => {
    if (!blockSwitch && sortedResults && sortedResults.length > visibleData.length) {
      const nextPage = page + 1;
      setPage(nextPage);
      const nextBatch = sortedResults.slice(size * nextPage, size * (nextPage + 1));
    ...
    <div className="GA-CCs" ref={currentComponentRef}>
      ...
      {sortedResults && (
        <div>
          ...
          <div>{visibleData.length} / {sortedResults.length}</div>
        </div>
      )}
    </div>
  );
